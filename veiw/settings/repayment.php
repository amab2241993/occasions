<?php
	...
	// if (isset($_SESSION['user_name'])){
		...
		$getH3     = 'سداد سلفيات';
		include '../../init.php';
		?><script src="<?php echo $controller ?>settings/repayment.js"></script><?php
		include $tpl . 'navbar.php';
		$stmt = $con->prepare("SELECT * FROM loans WHERE (Lending - repayment != 0) ORDER BY id ASC");
		$stmt->execute();
		$rows = $stmt->fetchAll();
		/* Start bocking Page */
	?>
		...
		<div class="col-4 mb-2 mt-4">
			...
			<select class="form-control" id="loan" required>
				...
				<?php
					foreach($rows as $row){
					?>
					<option value="<?=$row['id']?>" Lending="<?=$row['Lending']?>" repayment="<?=$row['repayment']?>"><?=$row['name']?></option>
					<?php
					}
				?>
			</select>
